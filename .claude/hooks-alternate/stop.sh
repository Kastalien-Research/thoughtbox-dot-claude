#!/usr/bin/env bash
# Stop hook - logs stop events and optionally announces completion
...
# Handle --chat switch: convert .jsonl to JSON array
if [[ "$chat_flag" == "true" ]] && [[ -n "$transcript_path" ]] && [[ -f "$transcript_path" ]]; then
    ...
    # Write to chat.json with formatting
...
# Get LLM-generated completion message
get_llm_completion_message() {
    local script_dir="$(dirname "$0")"
    local llm_dir="$script_dir/utils/llm"
    
    # Try OpenAI first
    if [[ -n "$OPENAI_API_KEY" ]] && [[ -f "$llm_dir/oai.sh" ]]; then
        msg=$(timeout 10s "$llm_dir/oai.sh" --completion 2>/dev/null || true)
        if [[ -n "$msg" ]]; then
            echo "$msg"
            return 0
        fi
    ...
    if [[ -n "$ANTHROPIC_API_KEY" ]] && [[ -f "$llm_dir/anth.sh" ]]; then
        msg=$(timeout 10s "$llm_dir/anth.sh" --completion 2>/dev/null || true)
        if [[ -n "$msg" ]]; then
            echo "$msg"
            return 0
        fi
    ...
    # Try Ollama third
    if [[ -f "$llm_dir/ollama.sh" ]]; then
        msg=$(timeout 10s "$llm_dir/ollama.sh" --completion 2>/dev/null || true)
        if [[ -n "$msg" ]]; then
            echo "$msg"
            return 0
        fi
...
if [[ "$notify_flag" == "true" ]]; then
    ...
    if [[ -n "$tts_script" ]]; then
        ...
        timeout 10s "$tts_script" "$completion_message" 2>/dev/null || true
    fi
fi
